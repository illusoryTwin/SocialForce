...
# def simulate(key_id=0, relative_id=1, vector_magnitude=2.0, return_vectors=False):
#     df = pd.read_csv("trajectories.csv")
#     df["timestamp"] = pd.to_datetime(df["timestamp"])
#     df.sort_values(by=["timestamp", "id"], inplace=True)
...
#             if return_vectors:
#                 vectors.append((frame_index, dx, dy))
...
#             else:
#                 vector_arrow = None
#             print(f"Frame {frame_index}: dx={dx}, dy={dy}, vector_magnitude={vector_magnitude}")
...
def simulate(key_id=0, relative_id=1):
